...
function makeDeps(leads: ExpiredLead[], overrides?: Partial<PurgeDeps>): PurgeDeps {
  return {
...
describe("purgeExpiredLeads", () => {
  it("returns zeros when nothing is expired", async () => {
    ...
    expect(result).toEqual({ leadsPurged: 0, filesDeleted: 0, filesFailed: 0, leadsSkipped: 0 });
    expect(deps.deleteLeads).not.toHaveBeenCalled();
  });

  it("deletes files then lead rows", async () => {
    const deps = makeDeps([
      { id: "a", attachments: [{ url: "/uploads/1.pdf" }, { url: "/uploads/2.jpg" }] },
      { id: "b", attachments: [] },
    ]);
    const result = await purgeExpiredLeads(now, deps);
    expect(result).toEqual({ leadsPurged: 2, filesDeleted: 2, filesFailed: 0, leadsSkipped: 0 });
  ...
  it("keeps a lead for retry when one of its files fails to delete", async () => {
    ...
    const result = await purgeExpiredLeads(now, deps);
    expect(result).toEqual({ leadsPurged: 1, filesDeleted: 1, filesFailed: 1, leadsSkipped: 1 });
