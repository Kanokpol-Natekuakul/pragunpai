...
describe("uploadAttachment", () => {
  ...
  it("should throw an error for unsupported mime-types", async () => {
    const file = new File(["test content"], "exploit.pdf", { type: "application/octet-stream" });
  ...
  it("should throw an error for unsupported extensions even if mime-type is spoofed", async () => {
    const file = new File(["test content"], "exploit.php", { type: "application/pdf" });
    await expect(uploadAttachment(file)).rejects.toThrow("ไม่รองรับนามสกุลไฟล์");
  });
