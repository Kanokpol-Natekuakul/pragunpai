...
export function InsurancePageEditor({ page }: InsurancePageEditorProps) {
  ...
  const [brochures, setBrochures] = useState<BrochureItem[]>(
    parseInitialPdfUrl(page.pdfUrl)
  );
  ...
  const handleSaveContent = () => {
    ...
    startTransition(async () => {
      ...
      const pdfUrlValue =
        activeBrochures.length > 0 ? JSON.stringify(activeBrochures) : null;
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-white border border-gray-200 space-y-6">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            <label className="block text-sm font-bold text-navy-800 mb-1">
              เบี้ยประกันเริ่มต้น (แสดงหน้าบัตร){" "}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="block text-sm font-bold text-navy-800 mb-1">
              รายละเอียดหลักประกัน / ความคุ้มครอง (Markdown/Text){" "}
              <span className="text-red-500">*</span>
            </label>
            <textarea
              rows={10}
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-xs text-navy-450 mt-1">
                กรอกชื่อป้ายกำกับของแต่ละไฟล์ เช่น &quot;โบรชัวร์รวม&quot;,
                &quot;แผนประกันสำหรับเด็ก&quot; และอัปโหลดไฟล์ PDF (ขนาดไม่เกิน
                5MB ต่อไฟล์)
              </p>
            </div>
            {brochures.length < 3 && (
                ...
                >
                  <div className="flex-1 w-full space-y-1.5">
                    ...
                    <input
                      ...
                      value={brochure.name}
                      onChange={(e) =>
                        handleBrochureNameChange(idx, e.target.value)
                      }
