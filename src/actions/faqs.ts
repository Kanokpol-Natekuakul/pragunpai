...
const validCategories = new Set<FaqCategory>(
  FAQ_CATEGORIES.map((item) => item.category)
);
    ...
    async () => {
      ...
      await prisma.$transaction(async (tx) => {
        await tx.siteSetting.upsert({
          ...
          create: {
            key: "faqSections",
            value: nextSettings as Prisma.InputJsonValue,
          },
        });
